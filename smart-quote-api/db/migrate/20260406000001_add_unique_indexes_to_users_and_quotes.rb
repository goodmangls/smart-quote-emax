class AddUniqueIndexesToUsersAndQuotes < ActiveRecord::Migration[8.0]
  def change
    ...
    remove_index :quotes, :reference_no, if_exists: true
    add_index :quotes, :reference_no, unique: true, if_not_exists: true
    ...
    add_index :quotes, [ :status, :validity_date ], name: "index_quotes_on_status_and_validity_date", if_not_exists: true
    add_index :quotes, :destination_country, if_not_exists: true
    add_index :quotes, [ :user_id, :created_at ], name: "index_quotes_on_user_id_and_created_at", if_not_exists: true
  end
end
