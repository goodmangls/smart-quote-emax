...
describe('fsc-history', () => {
  ...
  describe('loadFscHistory', () => {
    ...
    it('falls back to empty OCS array for legacy localStorage without ocs key', () => {
      ...
      expect(result.ups).toEqual(
        expect.arrayContaining([legacy.ups[0], { date: '2026-07-06', rate: 39.0 }]),
      );
    });
  ...
  describe('DEFAULT_FSC_HISTORY', () => {
    ...
    it('includes the latest confirmed FSC seed entries for all percentage carriers', () => {
      ...
      // OCS 는 ad-hoc 주기라 주간 갱신에 따라 움직이지 않는다.
      expect(DEFAULT_FSC_HISTORY.ocs.at(-1)).toEqual({ date: '2026-07-20', rate: 25.0 });
    });

    // 시드와 폴백 상수는 같은 주간 수치의 두 사본이다. 위 체크포인트는 시드만 보므로
    // rates.ts 를 빠뜨린 부분 갱신은 잡지 못한다 — tail 을 상수와 직접 대조한다.
    it('agrees with the fallback constants in rates.ts', () => {
      ...
      expect(DEFAULT_FSC_HISTORY.ocs.at(-1)?.rate).toBe(DEFAULT_FSC_PERCENT_OCS);
    });
  });
});
