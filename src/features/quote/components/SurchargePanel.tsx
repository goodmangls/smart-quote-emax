...
export const SurchargePanel: React.FC<Props> = ({
  ...
  const inputClass = `w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:border-emax-500 focus:ring-emax-500 border bg-white dark:bg-gray-700 dark:text-white transition-colors placeholder-gray-400 ${isMobileView ? 'text-base py-3.5 px-4' : 'text-sm py-2 px-3'}`;
    ...
    <div className='col-span-1 md:col-span-2'>
          ...
          <table className='w-full text-xs'>
            ...
            <tbody>
              {appliedSurcharges.map((s) => (
                <tr
                  key={s.code}
                  className='border-t border-gray-100 dark:border-gray-600 odd:bg-white even:bg-gray-50 dark:odd:bg-gray-800 dark:even:bg-gray-700/50'
                >
                    ...
                    <div className='flex items-center gap-1'>
                      ...
                      {s.sourceUrl && (
                        <a
                          ...
                          className='text-gray-400 hover:text-emax-500 transition-colors flex-shrink-0'
              ...
              {appliedSurcharges.length > 0 && (
                <tr className='border-t-2 border-gray-200 dark:border-gray-500 bg-amber-50 dark:bg-amber-900/10'>
                  <td
                    colSpan={2}
                    className='px-3 py-1.5 text-xs font-semibold text-amber-700 dark:text-amber-400'
                  >
      ...
      <div className='mt-3'>
        ...
        <input
          ...
          onChange={(e) =>
            onManualSurgeChange(e.target.value === '' ? undefined : Number(e.target.value))
          }
          ...
          autoComplete='off'
        />
        <p className='mt-0.5 text-[10px] text-gray-400'>{t('calc.service.surcharge.manualHint')}</p>
      </div>
      ...
      <div className='mt-2 flex items-center gap-3 flex-wrap'>
        {carrierLink && (
          <a
            ...
            target='_blank'
            rel='noopener noreferrer'
            className='inline-flex items-center gap-1 text-[10px] font-medium text-emax-600 dark:text-emax-400 hover:text-emax-700 dark:hover:text-emax-300 transition-colors'
          >
            <ExternalLink className='w-3 h-3' />
            {carrierLink.label}
          </a>
        ...
        {lastUpdated && !error && (
          <span className='text-[10px] text-gray-400'>
            {t('calc.service.surcharge.updated')}:{' '}
