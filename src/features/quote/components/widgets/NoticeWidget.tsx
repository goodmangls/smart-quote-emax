  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden transition-colors duration-200">
        ...
        <div className="p-0">
            ...
            {!loading && !error && (
              <>
                <ul className="divide-y divide-gray-100 dark:divide-emax-700">
                    {data.length === 0 ? (
                      ...
                      pageData.map((item, idx) => (
                        <li key={`${safePage}-${idx}`}>
                          <a
                            ...
                            className="flex items-start px-5 py-4 hover:bg-gray-50 dark:hover:bg-emax-700/50 transition-colors group"
                          >
                            <div className="mt-0.5 mr-3 flex-shrink-0">
                              <Newspaper className="w-4 h-4 text-emax-500" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate group-hover:text-emax-600 dark:group-hover:text-emax-400 transition-colors">
                                {item.title}
                              </p>
                            ...
                            <ExternalLink className="w-3.5 h-3.5 text-gray-300 dark:text-gray-600 shrink-0 mt-1 group-hover:text-emax-500 transition-colors" />
                          </a>
                        </li>
                      ))
                    )}
                ...
                {needsPagination && (
                  <div className="flex items-center justify-center gap-2 py-3 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-700/30">
                    <button
                      type="button"
                      onClick={prevPage}
                      className="p-2 sm:p-1 rounded-full hover:bg-gray-200 dark:hover:bg-emax-600 text-gray-400 dark:text-gray-400 transition-colors"
                      ...
                      {Array.from({ length: totalPages }, (_, i) => (
                        <button
                          ...
                          className={`w-2 h-2 rounded-full transition-all duration-200 ${
                            i === safePage
                              ? 'bg-emax-500 w-4'
                              : 'bg-gray-300 dark:bg-emax-600 hover:bg-gray-400 dark:hover:bg-emax-500'
                          }`}
                        />
                      ))}
                    </div>
